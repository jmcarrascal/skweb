...
public class ServicesAction extends ActionSupport {
	...
	public String preparedEstadistica() {
		...
		List<EstadiTipoReporte> tipoReporteList = transaccionManager
				.getEstadiTipoReporte();

		// Lista de TipoComprob
		ActionContext.getContext().getSession()
				.put("tipoReporteList", tipoReporteList);

		if (getUsuarioSesion().getRol() == Constants.ID_USR_VENDEDOR) {
			// Cargo los valores de Tipo de Reporte
			List<GenteBasic> clienteList = articuloManager
					.getClienteBasicPorVendedor(getUsuarioSesion()
							.getVendedorNr());
			// Lista de TipoComprob
			ActionContext.getContext().getSession()
					.put("clienteList", clienteList);
		}

		if (getUsuarioSesion().getRol() == Constants.ID_USR_CLIENTE) {
			// Cargo los valores de Tipo de Reporte
			List<Gente> clienteList = new ArrayList<Gente>();
			Gente gente = articuloManager.getGentePorPK(getUsuarioSesion()
					.getGenteNr());
		...
		if (getUsuarioSesion().getGenteSession() != null) {
			datosReporte.setAgendadoDesde(getUsuarioSesion().getGenteSession()
					.getGenteNr());
			datosReporte.setAgendadoHasta(getUsuarioSesion().getGenteSession()
					.getGenteNr());
		}
		return "success";
	}
	...
	public String menuAgendado_Tran() throws Exception {
		...
		if (gente.getGenteNr() == null) {
			...
			genteTmp = transaccionManager.getGenteConDomiPrinc(gente
					.getGenteNr());
		...
		List<TipoComprob> tipoComrobList = transaccionManager
				.getTipoComprobEnable(usuario.getRol());

		// Lista de TipoComprob
		ActionContext.getContext().getSession()
				.put("tipoComrobList", tipoComrobList);

		// Cargo los valores de todos los comprobantes
		List<TipoComprob> tipoComprobAllList = transaccionManager
				.getTipoComprobAll();

		// Lista de TipoComprob
		ActionContext.getContext().getSession()
				.put("tipoComprobAllList", tipoComprobAllList);

		// Cargo los valores de Tipo de Reporte
		List<TipoReporte> tipoReporteList = transaccionManager
				.getTipoReporteAll();
		...
		// Cargo los valores de Tipo de Reporte
		List<TipoReporte> tipoRepoTesoList = transaccionManager
				.getTipoReporteTesoreria();

		// Lista de TipoComprob
		ActionContext.getContext().getSession()
				.put("tipoRepoTesoList", tipoRepoTesoList);

		// Cargo los valores de Tipo de Reporte Estadistica
		List<TipoReporte> tipoRepoEstadisList = transaccionManager
				.getTipoReporteEstadistica();

		// Lista de TipoComprob
		ActionContext.getContext().getSession()
				.put("tipoRepoEstadisList", tipoRepoEstadisList);
	 ...
	 * 
	 */
	public String preparedFindAgendadoPorNombre_Resumen_CC_P_C()
			throws Exception {
		Usuario usuario = getUsuarioSesion();
		if (usuario.getRol() == Constants.ID_USR_GERENTE) {
			return "buscoAgendado";
	...
	public String preparedFindAgendadoPorNombre_Tran() throws Exception {
		...
		ActionContext.getContext().getSession()
				.put("tipoRepoEstadisList", tipoRepoEstadisList);
		if (getUsuarioSesion().getRol() == Constants.ID_USR_CLIENTE) {
			gente = articuloManager.getGentePorPK(getUsuarioSesion()
					.getGenteNr());
			return "goTransac";
		}
		return "success";
	...
	public String preparedFindGentePorNombre_CC_C() throws Exception {
		// Preguntar si el Rol es proveedor, por S Tomar el genteNr y envialo
		// directo Pro No ir a buscar el
		Usuario usuario = getUsuarioSesion();
	...
	public String findArticuloPorNombre() throws Exception {
		...
		List<Stock> articuloList = articuloManager.getArticuloProNombre(
				nombreArticulo, articuloActivo);
	...
	private String getAutoCompleteDataJquery(List<Stock> articuloList) {
		...
		for (Stock stock : articuloList) {
			i++;
			String descrip = stock.getDescripC().replaceAll("\"", "'")
					.replaceAll(",", " ");
			String tmp = "{ \"clave\": " + "\"" + stock.getClave()
					+ "\", \"valor\": " + "\"" + descrip + "\" }";
			if (total == i) {
			} else {
				tmp = tmp + ", ";
	...
	private String getAutoCompleteDataJqueryGente(List<Gente> genteList) {
		...
		for (Gente gente : genteList) {
			i++;
			String descrip = gente.getDescripC().replaceAll("\"", "'")
					.replaceAll(",", " ");
			String tmp = "{ \"clave\": " + "\"" + gente.getGenteNr()
					+ "\", \"valor\": " + "\"" + descrip + "\" }";
			if (total == i) {
			} else {
				tmp = tmp + ", ";
	 ...
	 * y data en el vector 1
	 */
	private String getAutoCompleteDataJqueryGenteBasic(
			List<GenteBasic> genteList) {
		String data = "";
		int i = 0;
		if (genteList != null) {
			int total = genteList.size();
	...
	public String vaciarCarrito_Carr() throws Exception {
		...
		List<Items> itemsSessionList = (List<Items>) request.getSession()
				.getAttribute("items_OP");

		List<Items> itemsCarritoSessionList = (List<Items>) request
				.getSession().getAttribute("items_CARR");

		if (itemsCarritoSessionList != null
				&& itemsCarritoSessionList.size() > 0) {
			for (Items items_carr : itemsCarritoSessionList) {
				itemsSessionList = transaccionManager.addItem(itemsSessionList,
						items_carr, getUsuarioSesion(), true);
			}
		}
		...
		Transac transacOrig = (Transac) request.getSession().getAttribute(
				"transac_tran");
	...
	public String changeValorMasivo() throws Exception {
		...
		itemsSessionList = transaccionManager.changeValorMasivo(
				itemsSessionList, items, tipoValorMasivo, itemsSelecionados,
				getUsuarioSesion());
	...
	public String addItem_OP() throws Exception {
		...
		String cadena = (String) request.getSession().getAttribute(
				"formatCadena");
		...
		if (cadena == null) {
			if (items.getBonif().equals(new BigDecimal(0))) {
				...
				cadena = FormatUtil.llenoConCeros(
						String.valueOf(items.getBonif().intValue()), 2)
						+ "+00+00+00";
			}
		}
		...
		if (items.getId().getItemNr() != null && items.getId().getItemNr() != 0) {
			itemsSessionList = transaccionManager.editItem(itemsSessionList,
					items, getUsuarioSesion());
		} else {
			// pregunto si la preferencia es de logica de Stock por piezas
			Double usaLogicaStockColor = PreferenciasUtil.comparePreferencia(
					getUsuarioSesion().getListPreferencias(),
					Constants.PREF_ID_USA_LOGICA_STOCK_COLOR);
			if (usaLogicaStockColor != null && usaLogicaStockColor == -1d) {
				// Obtengo las piezas ya precargadas
				List<StockPiezas> stockPiezasList = (List<StockPiezas>) request
						.getSession().getAttribute("stockPiezasList");
					...
					for (String str : piezas) {
						...
						try {
							...
							if (cantidad > 0d) {
								...
								// itemsNew = items;
								itemsNew.setColores(stockPiezasList.get(i)
										.getId().getColores());
								itemsNew.setCant1(new BigDecimal(cantidad));
								itemsSessionList = transaccionManager.addItem(
										itemsSessionList, itemsNew,
										getUsuarioSesion(), false);
							}
	...
	public String addItem_Carr() throws Exception {
		...
		// Transac transacOrig =
		// (Transac)request.getSession().getAttribute("transac_tran");
	...
	public String uploadArticuloImage() throws Exception {
		...
		String result = articuloManager.updateArticulo(stock.getClave(),
				stock.getImageFile());
		...
		List<ExiArt> exiArtList = articuloManager.getExistenciaPorArticulo(
				stock.getClave(), getUsuarioSesion());
	...
	public String saveTransac() throws Exception {
		...
		String result = transaccionManager.saveTransac_OP(itemsSessionList,
				gente.getGenteNr(), getUsuarioSesion().getEmpresaNrSk(),
				fechaEntrega, transac, transacOrig.getTipoComprob().getNr(),
				getUsuarioSesion());

		List<Items> itemsCarritoSessionList = new ArrayList<Items>();

		request.getSession()
				.setAttribute("items_CARR", itemsCarritoSessionList);
	...
	public String getArrayImagenes() throws Exception {
		...
		// articuloManager.getImagenArticuloPorFliaSubFlia(fam.getNrfam(),
		// subFam.getNrsubfam(), true);
		...
		for (Stock stock : usuario.getArticulosTmpList()) {
			...
			if (primero) {
				// result = "[";
				result = result + "downLoadImageArticulo?stock.clave=" + clave
						+ "&s=.jpg";
				primero = false;
			} else {
				result = result + "," + "downLoadImageArticulo?stock.clave="
						+ clave + "&s=.jpg";
			}
		}
		// result = result + "]";
	...
	public String deleteItemCarr() throws Exception {
		...
		itemsSessionList = transaccionManager.removeItem(itemsSessionList,
				items);
	...
	public String adminComprob_Tran() throws Exception {
		...
		if (transac.getTipoComprob() == null) {
			tipoComprob = transaccionManager
					.getTipoComprobByPK(Constants.ID_TIPO_COMP_PEDIDO_VENTA);
		} else {
			tipoComprob = transaccionManager.getTipoComprobByPK(transac
					.getTipoComprob().getNr());
		}
		...
		if (getUsuarioSesion().getRol() == Constants.ID_USR_CLIENTE) {
			...
			if (itemsCarritoSessionList != null
				...
				for (Items items_carr : itemsCarritoSessionList) {
					itemsSessionList = transaccionManager.addItem(
							itemsSessionList, items_carr, getUsuarioSesion(),
							true);
				}
			}
	...
	public String finComprob_Tran() throws Exception {
		...
		List<Domicilios> domiciliosList = transaccionManager
				.getDomiciliosPorGente(gente);

		// Lista de Domicilios
		ActionContext.getContext().getSession()
				.put("domiciliosList", domiciliosList);
	...
	public String getAgendadoPorCod_Tran() throws Exception {
		...
		try {
			nombreAgendado = articuloManager.getGentePorPKVendedor(
					gente.getGenteNr(), getUsuarioSesion()).getDescripC();
	...
	public String getOptionsSelect() throws Exception {
		...
		List<OptionsSelect> optionsList = articuloManager.getOptionsSelect(
				clave, idCombo, usaSeleccion);
	...
	public String getArticuloPorClave_OP() throws Exception {
		...
		// String artDescrip =
		// transaccionManager.getArticuloPorClave(stock.getClave(),
		// articuloActivo);

		// Obtengo la lista de Articulos
		List<Stock> stockList = articuloManager.getArticuloPorCodigoLike(
				stock.getClave(), articuloActivo, getUsuarioSesion());
	...
	public String getBonifPorCadena() throws Exception {
		...
		BigDecimal bonif = transaccionManager.getBonifPorCadena(bonif1, bonif2,
				bonif3, bonif4);

		String formatCadena = transaccionManager.getFormatCadena(bonif1,
				bonif2, bonif3, bonif4);
	...
	public String getPrecioPorArtAgendado() throws Exception {
		...
		BigDecimal precio = transaccionManager.getPrecioClienteArticulo(
				gente.getGenteNr(), stock.getClave(), usuario, false);
	...
	public String getFormulaStock() throws Exception {
		...
		if (PreferenciasUtil.comparePreferencia(getUsuarioSesion()
				.getListPreferencias(),
				Constants.PREF_ID_USA_LOGICA_STOCK_COLOR) == -1d) {
			List<StockPiezas> stockPiezasList = articuloManager
					.getStockPiezasPorArt(stock.getClave(), getUsuarioSesion());
			Double muestraCantidad = PreferenciasUtil.comparePreferencia(
					getUsuarioSesion().getListPreferencias(),
					Constants.PREF_ID_MUESTRA_CANTIDAD);
			// Obtengo Compras y Ventas
			// Obtengo la lista de las compras del articulo
			List<GroupCantTransac> compras = articuloManager
					.getOperacionByArtTipoCompr(stock.getClave(),
							Constants.ID_TIPO_COMP_PEDIDO_COMPRA);
			// Obtengo la lista de ventas del articulo
			List<GroupCantTransac> ventas = articuloManager
					.getOperacionByArtTipoCompr(stock.getClave(),
							Constants.ID_TIPO_COMP_PEDIDO_VENTA);
			List<GroupCantTransac> ventasPendientes = ventas;
			// Recorro compras
			for (GroupCantTransac compra : compras) {
				// pregunto si tengo pedido de venta
				GroupCantTransac ventaObtenida = new GroupCantTransac(
						BigDecimal.ZERO, compra.getColo());
				...
				if (!tieneStock) {
					...
					if (colores != null) {
						...
						stockpieza.setCant1(0d);
						stockpieza.setComprasmenosventas(compra.getCant1()
			...
			result = FormatUtil.getListHTMLStockPiezas(stockPiezasList,
					muestraCantidad, false, false, null, muestroBarraLateral);
			...
			result = result
					+ articuloManager.getTableResumenPedidos(stock.getClave(),
							getUsuarioSesion(), 3);

			result = result
					+ "<h4>Precio: $"
					+ transaccionManager.getPrecioClienteArticulo(genteNr,
							stock.getClave(), getUsuarioSesion(), false)
					+ "</h4>";
		} else {
			result = articuloManager.getFormulaStock(stock.getClave(),
					getUsuarioSesion(), carrito);
	...
	public String getTablePedido() throws Exception {
		...
		String result = transaccionManager
				.getTransacJoin(transac.getTipoComprob().getNr(),
						stock.getClave(), getUsuarioSesion());
	...
	public String getTableResumenPedido() throws Exception {
		...
		String result = articuloManager.getTableResumenPedidos(
				stock.getClave(), getUsuarioSesion(), 0);
	...
	public String sendComprobCliente() throws Exception {
		...
		Transac transacN = transaccionManager.getTransacByPK(transac
				.getTransacNr());

		// Obtengo el domicilio que tiene como descripcion @adm
		Domicilios domicilios = transaccionManager.getDomiciliosAdm(transacN
				.getGente());

		if (domicilios != null && domicilios.getInternet() != null
				&& !domicilios.getInternet().trim().equals("")) {

			// Obtengo el documento
			File fileDownload = cuentaCorrienteManager.getComprobantePdf(
					transac.getTransacNr(), getUsuarioSesion());

			// Obtener la direcci�n de correo del destinatario
			Email email = new Email("Simpa - Comprobante Electronico",
					"Se adjunta el Comprobante tipo: "
							+ transacN.getTipoComprob().getDescripcion()
							+ " Numero : " + transacN.getPrefijo() + "-"
							+ transacN.getNrComprob(), fileDownload,
					domicilios.getInternet());

			usuarioManager.sendMail(email);

			result = "<div class = space/>"
					+ "<div class = space/>"
					+ "<b>Se ha enviado el correo con �xito al la direcci�n de correo "
					+ domicilios.getInternet() + "</b>";

		} else {
			result = "<div class = space/>"
					+ "<div class = space/>"
					+ "<b>El agendado "
					+ transacN.getGente().getRazonSocial()
					+ " no tiene cargada una direcci�n de correo electr�nico en su domicilio \"@adm\"</b>";
		}
	...
	public String getAgendadoPorNombre_Tran() throws Exception {
		...
		List<GenteBasic> genteList = articuloManager
				.getProveedorBasicPorNombre(filter, getUsuarioSesion());
	...
	public String getItemsCarrito() throws Exception {
		...
		List<Items> itemsList = (List<Items>) request.getSession()
				.getAttribute("items_CARR");
		...
		salida.append(FormatUtil.getTableHTMLItemsCarrito(itemsList,
				getUsuarioSesion().getGenteSession().getRazonSocial(), transac));
	...
	public String getStockPorPiezasList() throws Exception {
		...
		List<StockPiezas> stockPiezasList = articuloManager
				.getStockPiezasPorArt(filter, getUsuarioSesion());
		String clave = "";
		if (stockPiezasList != null && stockPiezasList.size() > 0) {
		...
		Double usaLogicaPiezas = PreferenciasUtil.comparePreferencia(
				getUsuarioSesion().getListPreferencias(),
				Constants.PREF_ID_MUESTRA_CANTIDAD);
		if (carrito != null && carrito == 1) {
			salida.append(FormatUtil.getListHTMLStockPiezas(stockPiezasList,
					usaLogicaPiezas, true, true, request.getContextPath(),
					muestroBarraLateral));
		} else {
			salida.append(FormatUtil.getListHTMLStockPiezas(stockPiezasList,
					usaLogicaPiezas, true, false, null, muestroBarraLateral));
			salida.append(articuloManager.getTableResumenPedidos(clave,
					getUsuarioSesion(), 3));
		}
	...
	public String getStockPorNombreList() throws Exception {
		...
		List<Stock> stockList = articuloManager.getArticuloProNombre(filter,
				articuloActivo);
	...
	public String getSoloImpagoClientes() throws Exception {
		...
		List<GenteBasic> clienteList = articuloManager
				.getClienteBasicPorVendedor(getUsuarioSesion().getVendedorNr());
		...
		for (GenteBasic genteBasic : clienteList) {
			...
			SaldoAcumulado saldoAcumulado = cuentaCorrienteManager
					.getCuentaCorrienteVentasPorGente(genteBasic.getGenteNr(),
							getUsuarioSesion(), false);
			...
			if (saldoAcumulado.getListTransac().size() > 0) {
				if (saldoAcumulado.getFinAcumulado() > 1) {
					...
					for (Transac transac : saldoAcumulado.getListTransac()) {
						...
						transacReport.setSaldoArrastre(transac
								.getSaldoAcumulado());
						transacReport.setSaldoComprob(transac
								.getSaldoCalculado());
						transacReport.setTipoComprob(transac.getTipoComprob()
								.getDescripcion());
						transacReport.setTotalComprob(transac
								.getTotalCalculado());
						transacReportList.add(transacReport);
					}

					saldoAcumuladoReport.setFinAcumulado(saldoAcumulado
							.getFinAcumulado());
					saldoAcumuladoReport.setIncioAcumulado(saldoAcumulado
							.getIncioAcumulado());
	...
	public String getArticuloPorNombre() throws Exception {
		...
		List<Stock> articuloList = articuloManager.getArticuloProNombre(filter,
				articuloActivo);
	...
	public String showListArticulosPorArtMadre() throws Exception {
		...
		List<Stock> articuloList = articuloManager
				.getArticulosPorArtMadre(artMadre.getCodArtMad());
	...
	public String findArticuloPorProveedor() throws Exception {
		...
		List<Stock> articuloList = articuloManager.getArticuloPorProveedor(
				gente.getGenteNr(), articuloActivo);
	...
	public String findArtMadrePorNombre() throws Exception {
		...
		List<ArtMadre> artMadreList = articuloManager
				.getArtMadrePorNombre(artMadre.getDescArtMad());
	...
	public String generateEstadiReport() {
		...
		byte[] fileDownload = reportManager.generateEstadiReport(datosReporte,
				getUsuarioSesion());
		...
		try {
			...
			if (datosReporte.getFormatoReporte().equals("P")) {
				response.setHeader("Content-disposition",
						"attachment; filename=estadistica.pdf");

			}
			if (datosReporte.getFormatoReporte().equals("X")) {
				response.setHeader("Content-disposition",
						"attachment; filename=estadistica.xls");
			}
	...
	public String getSoloImpagosVentaPorGente_CC() throws Exception {
		...
		SaldoAcumulado saldoAcumulado = cuentaCorrienteManager
				.getCuentaCorrienteVentasPorGente(gente.getGenteNr(),
						getUsuarioSesion(), true);

		// totalSaldo =
		// cuentaCorrienteManager.getTotalSaldoVentasPorGente(saldoAcumulado.getListTransac());

		totalSaldo = saldoAcumulado.getFinAcumulado();

		request.setAttribute("transacList", saldoAcumulado.getListTransac());

		request.setAttribute("grillaResult",
				"El cliente: " + gente.getDescripC()
						+ " tiene un saldo total de: " + totalSaldo);
	...
	public String getPedPendientesPorGente_CC() throws Exception {
		...
		List<Transac> transacList = transaccionManager.getPedidosPendientes(
				gente, getUsuarioSesion());
	...
	public String getPedidoVtaPendiente() throws Exception {
		...
		List<Transac> transacList = transaccionManager
				.getPedidosVtaPendienteAprobar(gente, getUsuarioSesion());
	...
	public String getSoloImpagosPorGente_CC_C() throws Exception {
		...
		SaldoAcumulado saldoAcumulado = cuentaCorrienteManager
				.getCuentaCorrienteComprasPorGente(gente.getGenteNr(),
						getUsuarioSesion());
	...
	public String findPasebanPorAgendado_CR() throws Exception {
		...
		List<Paseban> pasebanList = tesoreriaManager
				.getPasebanPorAgendado(gente.getGenteNr());

		totalSaldo = tesoreriaManager.getPasebanTotalPorAgendado(gente
				.getGenteNr());
	...
	private HttpServletResponse getResponse() {
		return (HttpServletResponse) ActionContext.getContext()
				.getActionInvocation().getInvocationContext()
				.get(ServletActionContext.HTTP_RESPONSE);
	}
	...
	public String recuperarCheque() {
		...
		try {
			...
			try {
				...
				// response.setHeader("Content-disposition",
				// "attachment; filename=" +
				// facturaActual.getNombreArchivo().replaceAll("txt", "xml"));
				response.setHeader("Pragma", "Public");
	...
	public String reportePorTipoComprob_Repo() throws Exception {
		...
		List<Transac> transacList = cuentaCorrienteManager
				.getTransacPorTipoComprob(gente.getGenteNr(),
						getUsuarioSesion(), transac.getTipoComprob().getNr(),
						fechaDesde, fechaHasta);

		request.setAttribute("transacList", transacList);

		TipoComprob tipoComprob = transaccionManager.getTipoComprobByPK(transac
				.getTipoComprob().getNr());
	...
	public String resumen_CC() throws Exception {
		...
		SaldoAcumulado saldoAcumulado = cuentaCorrienteManager
				.getResumenCuentaCorriente(gente.getGenteNr(),
						getUsuarioSesion(), fechaDesde, fechaHasta);
	...
	public String estadisticaArtGente() throws Exception {
		...
		List<Items> itemsList = transaccionManager.getEstadisticaArtGente(
				gente.getGenteNr(), getUsuarioSesion(), fechaDesde, fechaHasta);
	...
	public String resumen_CC_C() throws Exception {
		...
		SaldoAcumulado saldoAcumulado = cuentaCorrienteManager
				.getResumenCuentaCorrienteCompras(gente.getGenteNr(),
						getUsuarioSesion(), fechaDesde, fechaHasta);
	...
	public String findArticuloPorFlia() throws Exception {
		...
		List<Stock> articulosTmpList = articuloManager
				.getArticuloPorFliaSubFlia(fam.getNrfam(),
						subFam.getNrsubfam(), articuloActivo,
						getUsuarioSesion());
	...
	public String findArticuloPorFlia_Carr() throws Exception {
		...
		String rows = context.getParameter("notReg_"
		...
		case 0:
			regTotalRows = articuloManager.getCountArticuloPorFliaSubFlia(
					fam.getNrfam(), subFam.getNrsubfam(), articuloActivo,
					propertySort, orderSort, propertyFilter, valueFilter, min,
					pageSize);
			articulosTmpList = articuloManager.getArticuloPorFliaSubFlia(
					fam.getNrfam(), subFam.getNrsubfam(), articuloActivo,
					propertySort, orderSort, propertyFilter, valueFilter, min,
					pageSize);
			break;
		case 1:
			regTotalRows = articuloManager.getCountArticuloPorNombre(
					stock.getDescripcion(), articuloActivo, propertySort,
					orderSort, propertyFilter, valueFilter, min, pageSize);
			articulosTmpList = articuloManager.getArticuloPorNombre(
					stock.getDescripcion(), articuloActivo, propertySort,
					orderSort, propertyFilter, valueFilter, min, pageSize);
			break;
		case 2:
			regTotalRows = articuloManager.getCountArticuloPorCodigo(
					stock.getClave(), articuloActivo, propertySort, orderSort,
					propertyFilter, valueFilter, min, pageSize);
			articulosTmpList = articuloManager.getArticuloPorCodigo(
					stock.getClave(), articuloActivo, propertySort, orderSort,
					propertyFilter, valueFilter, min, pageSize);
			break;
		}
		...
		for (Stock stock : articulosTmpList) {
			stock.setPrecioTemp(MathUtil.redondearEn2BD(transaccionManager
					.getPrecioClienteArticulo(getUsuarioSesion()
							.getGenteSession().getGenteNr(), stock.getClave(),
							usuario, false)));
	...
	public String downLoadImageArticulo() throws Exception {
		...
		ImagenesArticulos ia = articuloManager.getImageArticuloByPK(stock
				.getClave());
	...
	public String findGentePorNombre_CC() throws Exception {
		...
		if (usuario.getRol() == Constants.ID_USR_VENDEDOR) {
			genteList = articuloManager.getProveedorPorNombreVendedor(
					nombreProveedor, usuario);
		}
	...
	public String findProveedorPorNombre() throws Exception {
		...
		List<Gente> genteList = articuloManager
				.getProveedorPorNombre(nombreProveedor);
	...
	/**
	 * Este Metodo muestra los remitos pendientes de recepcionar por el usuario
	 * logeado de Trazabilidad
	 */
	public String preparedListRemitosPendientes() throws Exception {
		...
		List<Transac> transacList = transaccionManager
				.getRemitosPendientesPorClienteTraza(getUsuarioSesion());
	...
	/**
	 * Este Metodo muestra los Medicamentos pendientes de recepcionar por el
	 * usuario logeado de Trazabilidad
	 */
	public String viewTrazaIngreso() throws Exception {
		...
		List<Trazabi> trazabiList = transaccionManager
				.getTrazabiPorComprobante(transac.getTransacNr());
